...
- **`@llm-space/studio`** (`packages/studio`) — Studio application module over Engine. Owns ordinary `Playground`/`AgentSpec` metadata plus Project Experiment metadata, dirty editable Drafts, Run ordering, evaluations, and Studio event projection. Run state/messages remain in Engine checkpoints; a Draft is cleared when a Run starts. SQLite tables share one physical database with Engine but retain separate table ownership.
- **`@llm-space/example-basic-agent`** (`examples/basic-agent`) — locally runnable tracer bullet across loader → App → Engine → Pi Run executor → authored tools. `mise run dev:agent` uses a deterministic Pi faux model by default and persists Engine checkpoints plus Session history in SQLite under the LLM Space home.
