...
- **`@llm-space/engine`** (`packages/engine`) — the product-neutral Pi-backed document execution facade shared by Playground, Project Experiment, and App admission paths. It owns Step/Turn/Continue semantics, committed + ephemeral observation, tool permission/cancel delegation, Agent capability fail-fast validation, and host-injected prompt materialization. It depends inward on Agent/Core/Pi Runtime; UI, ACP transport, Studio/App metadata, SQLite composition, and Desktop product identity do not belong here.
...
- **`@llm-space/acp`** (`packages/acp`) — the isolated official ACP v2 Draft edge pinned to `@agentclientprotocol/sdk@1.3.0`. It projects committed Pi log identities, durable usage records, transient running state, and ephemeral model deltas into replay-safe ACP updates and hosts the standard session lifecycle plus capability-negotiated `_llm-space.dev/session/{snapshot,step,turn,continue}` methods. Pi Runtime never depends on ACP. CLI carries ACP over stdio; Desktop carries the same ACP updates over namespaced Electrobun RPC without nesting JSON-RPC.
- **`@llm-space/app`** (`packages/app`) — reusable Pi-backed end-application layer. It owns Session and Task product metadata; transcript, operation state, branch, leaf, outcome, and usage come from Pi Session entries. Idempotency uses Pi operation/action/tool identities rather than client command receipts. Its Bun-only `@llm-space/app/server` entry exposes `createAgent()`, which accepts `projectRoot`, host-owned `dataRoot`, Pi `Models`, and explicit runtime services, then composes Loader + Pi Runtime + App tables in one SQLite database.
- **`@llm-space/studio`** (`packages/studio`) — Studio product metadata over Pi Session. It owns ordinary `Playground`/`AgentSpec` and Project Experiment metadata, dirty editable Drafts, ordered Pi operation references, evaluations, and Studio event projection. Transcript, active operation, lane, leaf, outcome, and usage are read from Pi Session. Draft admission clears the Draft after Pi commits the operation. Pi, immutable runtime bindings, and Studio tables share one host-owned SQLite connection with separate table ownership. Its Bun-only `@llm-space/studio/server` entry owns Project source browsing/watch, Git revision binding, Agent loading, Pi runtime composition, and SQLite; Desktop exposes it only through namespaced Electrobun RPC.
...
evaluations, and events, while `acpSession.*` exclusively owns Prompt/Step/Turn/Continue,
...
Main-window and Project execution is `Playground/Experiment metadata + Pi Session`: UI action → Zustand ACP projection → namespaced Electrobun `acpSession.*` RPC → Playground/Studio application → `DocumentExecutionEngine` → `DurablePiRuntime` → Pi Session. The ACP Prompt carries the selected user content plus preceding committed ACP context; product metadata save RPCs must never become a second execution-message transport. ACP chunks, usage, state, and committed upserts update the UI projection; Studio stores only Drafts, product metadata, operation ordering/references, and evaluations. UI `Thread` is derived presentation/editing state, never an execution or transcript authority. Stateless helper generation uses the separate `auxiliaryGeneration.*` RPC and never creates a Pi Session.
...
Each open thread owns its own Zustand store (`stores/thread-store.ts`), created per-tab via `createThreadStore()` and supplied through `ThreadStoreContext` — there is **no global store**. Read it with `useThreadStore(selector)` and `useThreadStoreActions()`. Interactive stores hold one `AcpSessionProjection`; their `Thread` is a presentational/editable projection. Undo/redo lives in `stores/thread-history.ts`: snapshots are thread _references_ (copy-on-write shares unchanged substructure, so undo is an O(1) pointer move), capped by count and a retained-image-bytes budget.
...
- Old local JSON Threads and legacy Thread Snapshots are not migrated or reopened. Import accepts only `llm-space.shared-document@1` (Agent document plus committed ACP upserts) and creates a new Playground/Pi Session.
...
Interactive Thread editor execution (`prompt`, `step`, `turn`, `continue`, tool approval,
and cancellation) is the deliberate exception: these actions belong to the
injected `AcpSessionExecutionRuntime`, which calls the product-scoped
`acpSession.*` RPC namespace directly and folds ACP updates into the editor.
