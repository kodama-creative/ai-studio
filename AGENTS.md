...
Remote Pi-backed Agent execution uses the same ACP client semantics over one no-TTY `ssh <host> llm-space acp [--project <remote-path>]` stdio process (`bun/remote/ssh-acp-connection.ts`). The HTTP/tunnel Remote Runtime remains separate because it also owns remote models, settings, MCP, skills, tools, and filesystem capabilities.
