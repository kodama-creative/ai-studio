...
- **`@llm-space/engine`** (`packages/engine`) — retiring execution kernel used only by the not-yet-migrated App/CLI path. Studio and Desktop local execution no longer create Engine Thread, Run, or Checkpoint state. Phase 5 removes this package after App/CLI cutover.
- **`@llm-space/engine-pi`** (`packages/engine-pi`) — retiring Pi executor adapter for the remaining Engine-backed App/CLI path. Studio and Desktop do not use it; Phase 5 removes it with Engine.
- **`@llm-space/pi-runtime`** (`packages/pi-runtime`) — the durable Pi Session execution kernel. It owns the Bun-native, `pi_`-prefixed SQLite `SessionRepo`, immutable LLM Space runtime bindings, direct one-turn Pi model execution, and the non-inherited `DurablePiAgentHarness` manual/automatic driver. Pi `AgentMessage` is canonical; local tools are sequential and use explicit safe/never crash replay. Playground and Project Experiment execution already use this runtime as their only transcript and execution-state authority.
...
- **`@llm-space/studio`** (`packages/studio`) — Studio product metadata over Pi Session. It owns ordinary `Playground`/`AgentSpec` and Project Experiment metadata, dirty editable Drafts, ordered Pi operation references, evaluations, durable debugger command receipts, and Studio event projection. Transcript, active operation, branch, leaf, outcome, and usage are read from Pi Session. Draft admission clears the Draft after Pi commits the operation. Pi, immutable runtime bindings, and Studio tables share one physical SQLite database with separate table ownership. Its Bun-only `@llm-space/studio/server` entry owns Project source browsing/watch, Git revision binding, Agent loading, Pi runtime composition, and SQLite; Desktop adapts it to Project windows and ACP/RPC.
...
Local main-window and Project execution is now `Playground/Experiment metadata + Pi Session`: UI action → Zustand external execution runtime → official ACP v2 semantics over the Electrobun custom envelope → Studio application → `StudioPiSessionRuntime` → `DurablePiAgentHarness`. The UI refreshes committed transcript from Pi message entries; Studio stores only Drafts, product metadata, operation ordering/references, evaluations, and command receipts. Remote legacy workspaces still use the `AgentTransport`/`streamThread()` route; they are not local product persistence.
