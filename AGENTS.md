...
- **`@llm-space/engine`** (`packages/engine`) — headless durable Agent execution kernel. Owns Engine `Thread`, immutable `ThreadCheckpoint`/`ThreadState`, durable `Run`, Worker lease/recovery, Run events, and in-memory/SQLite stores. A `RunExecutor` executes one complete Agent loop and reports awaited semantic events; Engine commits model/tool Checkpoints before allowing execution to advance. `ThreadState.messages` directly uses `@llm-space/core.Message[]`; Engine has no Session, Task, Project, Studio, provider, or ReAct implementation concept.
- **`@llm-space/engine-pi`** (`packages/engine-pi`) — Pi implementation of the Engine `RunExecutor` seam. It uses `pi-agent-core.runAgentLoopContinue()` for the complete ReAct loop, adapts LLM Space tools and `ToolContext`, converts core/Pi messages, and awaits Engine event persistence between model/tool phases. It does not own Thread, Run, Checkpoint, Session, or persistence.
