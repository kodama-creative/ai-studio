...
- **`@llm-space/engine`** (`packages/engine`) — headless durable Agent execution kernel. Owns Engine `Thread`, immutable `ThreadCheckpoint`/`ThreadState`, durable user-triggered `Run`, Worker lease/recovery, step/continue control, Run events, and in-memory/SQLite stores. Engine chooses one model or tool Step at a time; Continue repeatedly advances the same Run, while Step pauses it after one committed Step. `ThreadState.messages` directly uses `@llm-space/core.Message[]`; Engine has no Session, Task, Project, Studio, provider, or Pi implementation concept.
- **`@llm-space/engine-pi`** (`packages/engine-pi`) — Pi implementation of the Engine `RunExecutor` seam. For a model Step it reuses `pi-agent-core.agentLoopContinue()` but terminates after the first model turn; for a tool Step it executes only the Engine-selected calls. It adapts LLM Space tools and `ToolContext`, converts core/Pi messages, and awaits Engine event persistence. It does not own Thread, Run, Checkpoint, Session, or persistence.
- **`@llm-space/pi-runtime`** (`packages/pi-runtime`) — the replacement durable Pi Session execution kernel under construction. It owns the Bun-native, `pi_`-prefixed SQLite `SessionRepo`, immutable LLM Space runtime bindings, direct one-turn Pi model execution, and the non-inherited `DurablePiAgentHarness` manual/automatic driver. Pi `AgentMessage` is canonical; local tools are sequential and use explicit safe/never crash replay. Phase 0–2 does not cut Studio/App/CLI over yet, so Engine and Engine-Pi remain as the retiring production path.
- **`@llm-space/acp`** (`packages/acp`) — the isolated official ACP v2 Draft edge pinned to `@agentclientprotocol/sdk@1.3.0`. It projects committed Pi log identities into replay-safe ACP updates and hosts the standard session lifecycle plus capability-negotiated `_llm-space.dev/session/{snapshot,step,continue}` methods. Pi Runtime never depends on ACP; hosts implement the product-owned session metadata/binding backend and choose stdio, SSH, or Electrobun transport.
- **`@llm-space/app`** (`packages/app`) — reusable end-application layer. Owns `Session`, durable `SessionMessage`, optional `Task`, `SessionRunLink`, Engine-to-Session projection, and in-memory/SQLite stores. `Session.threadId` is only the default continuation Thread; active work is derived from Engine Runs. Its Bun-only `@llm-space/app/server` entry exposes the high-level `createAgent()` facade, which accepts `projectRoot`, host-owned `dataRoot`, Pi `Models`, and explicit runtime services, then composes Loader + Engine Pi + Engine/App SQLite without exposing `RunExecutor` to the host.
- **`@llm-space/studio`** (`packages/studio`) — Studio application module over Engine. Owns ordinary `Playground`/`AgentSpec` metadata plus Project Experiment metadata, dirty editable Drafts, Run ordering, evaluations, and Studio event projection. Run state/messages remain in Engine checkpoints; a Draft is cleared when a Run starts. SQLite tables share one physical database with Engine but retain separate table ownership. Its Bun-only `@llm-space/studio/server` entry exposes `createStudio()` and also owns Project source browsing/watch, Git revision binding, Agent loading, Engine Pi, and SQLite; Desktop only adapts it to Project windows and RPC.
- **`@llm-space/cli`** (`packages/cli`) — terminal composition root. `llm-space dev` opens the installed Studio through a project deep link; `llm-space run` loads user model settings, creates the high-level App facade, and renders durable Run events. It does not host a second Agent server or TUI.
...
- **`@llm-space/core`** (`packages/core`) — domain library, **no build step**; its TypeScript is consumed directly via the `exports` map. Entrypoints:
  ...
  - `./types` — `Thread`/`Message`/`ModelConfig`/`Tool`/`FileNode`/`ModelProviderGroup`, the network/search settings shapes shared across the RPC boundary (`types/network`, `types/search`), and the converters to/from the `@earendil-works/pi-*` formats.
...
Local main-window execution is now `Playground + AgentSpec + Engine Thread`: UI action → Zustand external execution runtime → `PlaygroundClient` RPC → `PlaygroundApplication` → `AgentEngine` → `PiRunExecutor`. Engine RunFrames are mapped back into streaming messages and durable checkpoint-backed editor Threads. Remote legacy workspaces still use the `AgentTransport`/`streamThread()` route; they are not local product persistence.
...
- `studio/studio.sqlite` — local main-window Playgrounds. Engine owns `engine_*`; Studio owns `studio_*`.
- `studio/projects/<project-id>/studio.sqlite` — code-first Agent Project Experiments. The path is user application data, never written under project source.
- `workspace/` — legacy/import/export and remote workspace JSON support behind `fs*`; it is no longer the local main-window Playground source of truth.
...
- Old local JSON Threads and Harness data are not migrated or reopened as Playgrounds; users may explicitly import JSON. Upgrade code never broadly deletes data it does not own.
...
- `components/` — desktop-only UI: `thread-tabs/`, `file-system-tree-view/`, `settings/`, `command-palette.tsx`, `onboard-dialog.tsx`, `feature-reminder-dialog.tsx` (the "what's new" reminder popup), and the account/update/github widgets. **The Thread Playground, model-provider, code-editor, shadcn `ui/`, and design tokens moved to `@llm-space/ui`** — import them from there, not from `@/components`.
