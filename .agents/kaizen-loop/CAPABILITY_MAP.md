...
- Evidence:
  ...
  - Review-fix tests cover qualified-name collision rejection, in-flight call cancellation, Bun-side rejection of drifted calls, unavailable-tool model filtering, and a checked-in `apps/example-agent/agent/connections/fixture.ts` reference beside the local tool.
