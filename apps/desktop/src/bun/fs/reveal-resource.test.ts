...
import { revealResource } from "./reveal-resource";
...
describe("revealResource", () => {
  test("opens a directory itself", async () => {
    ...
    await revealResource(directory, {
  ...
  test("resolves an internal skill locator to the skill directory", async () => {
    ...
    await revealResource("llm-space://internal/skills/known-skill", {
      skillsManager,
      openPath,
    });
  ...
  test("rejects relative and missing paths", async () => {
    expect(
      revealResource("relative/path", { skillsManager: _skillsManager() })
    ).rejects.toThrow("Path must be absolute");

    const directory = await _temporaryDirectory();
    expect(
      revealResource(path.join(directory, "missing"), {
        skillsManager: _skillsManager(),
      })
  ...
  test("rejects invalid internal locators and unknown skills", () => {
    expect(
      revealResource("llm-space://shared/gist/threads/123", {
        skillsManager: _skillsManager(),
      })
    ).rejects.toThrow("Invalid internal resource locator");

    expect(
      revealResource("llm-space://internal/skills/unknown-skill", {
        skillsManager: _skillsManager(),
      })
